...
**Your Answers here:**
<br>https://github.com/dgerlanc/portfolio
<br>https://CRAN.R-project.org/package=AmericanCallOpt 
...
<br>1.The ticker AAPL is downloaded after the ticker SPY in line 8 of data Set 1 . In Rcode : mytickers = c("SPY", "AAPL")"\nDownloading ticker  AAPL  ..."

<br> 2.round(100*sum(StockReturns[,2],1)
[1] 596.5

<br> round(100*mean(StockReturns[,2]),3)
[1] 0.148

<br>round(100*sd(StockReturns[,2]),3)
[1] 2.389


3.<br>To get the data for Yahoo this part of the code should be modified in line 8 of dataSet1 mytickers = c("SPY", "AAPL","YHOO") 

<br>round(100*sum(StockReturns[,3]),3)
[1] 290.567 

<br>round(100*mean(StockReturns[,3]),3)
[1] 0.072 

<br>round(100*sd(StockReturns[,3]),3)
[1] 3.019
...
<br>1. ncol(transposedData)
[1] 4034

<br> nrow(transposedData)
[1] 3

<br>The rows of transposedata should be two if you use the original data with only SPY and AAPL. To isolate only required transposed data you should just consider a part of the matrix as is done in question 2.

<br> 2.transposedData = t(StockReturns[,c("SPY","AAPL")])
<br>porfolio = apply(transposedData,2,mean)
<br> names(portfolio) <- rownames(StockReturns[,c("SPY","AAPL")])
<br> pnl_plot(porfolio)
...
**Your Answers here:**
<br>1. You have to change the startDate to  "2000-12-29"in dataSet1.R and you got data starting from the 2nd of January since the first ofJanuary stock market is closed.
<br>2.Yes I have created a new porfolio adding GOOGLE and TESLA on the data set in the part of the code line 8 hereby described:
<br> mytickers = c("SPY", "AAPL","YHOO","GOOG","TSL").

<br> To select just a specified period of time of data you can built a porfolio with a time series of data of GOOGLE YAHOO SPY TESLA AND APPLE in the following way:
<br> transposedNewPortfolio = t(StockReturns[c(3000:4000),])
<br> NewPortfolio <- apply(transposedNewPortfolio,2,mean)
<br> names(NewPortfolio)<-rownames(StockReturns[c(3000:4000),])
<br> pnl_plot(NewPortfolio)
...
<br> 1.It gives you as a result 20 and tell you how many times the data in the matrix twentydays(myData) match the Stock returns database.

<br> 2. The two databases cannot be merged since they are in different extensions/format.Therefore we have an error message.
...
<br>Just for an example I load a data set with the city of loas angeles crimes in the period 2013-2015.
