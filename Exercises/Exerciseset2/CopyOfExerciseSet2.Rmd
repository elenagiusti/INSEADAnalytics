...
setwd("/Users/elena/INSEADAnalytics/INSEAD Analytics/Exercises/Exerciseset2")
list.files()
```
...
**Answers**
<br> 1. 5 components would explain appproximately 52% of overall variance.

<br> 2.5 yr T-Notes US 
10yr T-Notes
```{r}
...
```
<br> During year of bear market we notice that 50% of the variance is explained by only a small number of principal components (3) . This means that in the market during downstate asset are more strongly correalted and data can be explained by a lower number of unique variables.
...
<br>1. The code takes the moving average od all retrun each 20 days. Then it assign a relative weight of all asset up to 1 looking to previous aggregate result. Finally it project the result in the future.
