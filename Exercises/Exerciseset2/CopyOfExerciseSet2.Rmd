...
setwd("/Users/elena/INSEADAnalytics/Exercises/Exerciseset2")
list.files()
```
...
library(reshape2)
subset(melt(show_data), value > 0.8 & value < 1)
iprint.df(subset(melt(show_data), value > 0.8 & value < 1), scale=TRUE)
```

<br> It totally makes sense that most of the title are positively correlated and follow the overall market trend while there are no any title that are negatively correlated and perform in different way compared to the market.
<br>Most of commodities show strong positive correlation in particular those part of the same group ( e.g Brent and Oil price and Heating oil) taking an high limit of >0.8
<br>Taking a lower limit >0.7 the number of commodities positively correltated is much higher ( e.g Silver with Gold)
...
<br> Question 1 
Five components would explain appproximately 52% of overall variance.
...
```{r echo=TRUE, eval=TRUE}
max(futures_data[,"5 yr T-Notes US"])
max(futures_data[,"10yr T-Notes"])
```

<br>Question 3
<br> 1. Treasury Bond
<br> 2. Currency Gold and Silver
<br> 3 and 4 Market index
<br> 5. European index
<br> 6. Oil
<br> 7.Metals
<br> 8. Cereals
<br> 9.Precious metal
<br> 10. Asian markets index
...
```
<br> During year of bear market we notice that 50% of the variance is explained by only a small number of principal components (3) . This means that in the market during downstate assets are more strongly correalted and data can be explained by a lower number of unique variables.
...
<br>1. The code takes the moving average od all retruns in an interval of 20 days. Then it assigns a relative weight to all assets up to 1 looking to previous aggregate result. Finally it projects the result in the future.

<br>2.
```{r echo=TRUE, eval=TRUE}

MAfreq2<-seq(1,100,by=50)
for (iter in 1:length(MAfreq2))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq2[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
